...
- `src/argon2.ts` → `unsecure/argon2` — `argon2()`, `argon2Hash()`, `argon2Verify()`: Argon2 (RFC 9106), `argon2id` by default, in plain JavaScript (no WebAssembly, no native binding). `argon2Hash`/`argon2Verify` speak the PHC string format; only version `0x13` is produced or accepted, and an unknown variant/version or a malformed string throws rather than returning `false`.
