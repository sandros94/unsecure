...
Before, failures came back as `Error`, `TypeError`, `RangeError`, `SyntaxError`, or an opaque `DOMException` from Web Crypto. Now every one of them is an [`UnsecureError`](/errors) carrying a machine-readable `code`.
...
The `{ loose: true }` in the right column is not decoration. The old wrappers decoded leniently; `parse` is [strict by default](/codecs#strict-and-loose). Drop it only once you know the text is canonical.
...
- [`unsecure/argon2`](/password-hashing) — password hashing (RFC 9106) in plain JavaScript. `argon2Hash`, `argon2Verify`, `argon2NeedsRehash` and the raw `argon2` KDF.
- [`unsecure/errors`](/errors) — `UnsecureError` and `UnsecureErrorCode`.
- Algorithm names are matched case-insensitively everywhere, so `"sha-256"` works.
- `importHmacKey` and `importHkdfKey`, and `CryptoKey` accepted in place of a secret by `hmac`, `hmacVerify`, `hkdf` and the four OTP functions. See [Hashing and MAC](/hashing/hmac#reusing-an-imported-key).
