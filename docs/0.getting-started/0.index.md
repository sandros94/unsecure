...
| Family                                | Functions                                                                       |
| ------------------------------------- | ------------------------------------------------------------------------------- |
| [Hashing and MAC](/hashing)           | `hash`, `hmac`, `hmacVerify`, `importHmacKey`                                   |
| [Key derivation](/key-derivation)     | `hkdf`, `importHkdfKey`                                                         |
| [Password hashing](/password-hashing) | `argon2`, `argon2Hash`, `argon2Verify`, `argon2NeedsRehash`                     |
| [One-time passwords](/otp)            | `hotp`, `totp`, their verifies, `generateOTPSecret`, `otpauthURI`               |
| [UUID](/uuid)                         | `uuidv4`, `uuidv7`, `createUUIDv7Generator`, `uuidv7Timestamp`, the type guards |
| [Secrets](/secrets)                   | `secureGenerate`, `secureCompare`, `entropy`                                    |
| [Sanitize](/sanitize)                 | `sanitizeObject`, `sanitizeObjectCopy`, `safeJsonParse`                         |
| [Random](/random)                     | `secureRandomNumber`, `secureRandomBytes`, `secureShuffle`, `randomJitter`      |
| [Codecs](/codecs)                     | `Hex`, `Base64`, `Base32` and their flat `stringify` / `parse` functions        |
| [Errors](/errors)                     | `UnsecureError` and its code set                                                |
...
**One error class, one code set.** Every failure is an [`UnsecureError`](/errors) with a `code` you can branch on. A native `TypeError` escaping the library is a bug.
