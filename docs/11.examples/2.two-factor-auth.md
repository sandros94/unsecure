...
**Secret storage.** `otpSecret` is a bearer credential. Encrypt the column, or derive its encryption key with [`hkdf`](/key-derivation) from a key that is not in the database.
