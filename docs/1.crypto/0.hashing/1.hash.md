...
Both `UNSUPPORTED` checks run before Web Crypto is reached, so a typo costs nothing. See [Errors](/safety/errors).
...
A plain SHA-256 is the right tool here and the wrong tool for a password, because a token has full random entropy and a password does not. See [Password hashing](/crypto/password-hashing) for the other case.
...
A digest is not secret, but the value you compare it against often is. Use [`secureCompare`](/generate/secrets/compare) whenever the comparison decides whether a request is authorized.
