...
`hmac` follows the same rule as [`hash`](/crypto/hashing/hash): a `string` in gives hex out, bytes in give bytes out, an explicit `returnAs` wins.
...
`hmacVerify` computes the MAC once as raw bytes, then compares with [`secureCompare`](/generate/secrets/compare) in constant time.
