...
| ------------------------------------------------------------ | ------------------------------------------------- |
| Are these two blobs the same?                                | [`hash`](/crypto/hashing/hash)                    |
| Has this file changed since I last saw it?                   | [`hash`](/crypto/hashing/hash)                    |
| Can I store this API token without storing the token itself? | [`hash`](/crypto/hashing/hash)                    |
| Did this webhook really come from the provider?              | [`hmacVerify`](/crypto/hashing/hmac)              |
| Can I hand a client a value it cannot tamper with?           | [`hmac`](/crypto/hashing/hmac)                    |
| Can I store this user password?                              | No. Use [`argon2Hash`](/crypto/password-hashing). |
| Can I turn a shared secret into several keys?                | No. Use [`hkdf`](/crypto/key-derivation).         |
...
All of them are an [`UnsecureError`](/safety/errors).
