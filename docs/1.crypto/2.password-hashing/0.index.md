...
All three functions are `async` for symmetry with [`hash`](/crypto/hashing/hash) and [`hmac`](/crypto/hashing/hmac), but the derivation runs synchronously on the calling thread. The promise settles only after every block has been computed, and nothing else on that thread runs in the meantime.
