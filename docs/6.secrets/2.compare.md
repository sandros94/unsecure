...
[`hmacVerify`](/hashing/hmac), [`hotpVerify`, `totpVerify`](/otp) and [`argon2Verify`](/password-hashing) all use `secureCompare` internally. You do not need to call it yourself when you are using those.
