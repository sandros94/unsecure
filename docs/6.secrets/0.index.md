...
::card-group
::card{title="secureGenerate()" icon="i-lucide-wand-sparkles" to="/secrets/generate"}
Passwords, tokens and PINs from a CSPRNG, with per-category guarantees.
::
::card{title="secureCompare()" icon="i-lucide-equal" to="/secrets/compare"}
Constant-time comparison that treats untrusted input as a mismatch, not an error.
::
::card{title="entropy()" icon="i-lucide-activity" to="/secrets/entropy"}
